...
const modulePascal = (0, lodash_1.upperFirst)((0, lodash_1.camelCase)(moduleName));
const moduleCamel = (0, lodash_1.camelCase)(moduleName);
const moduleSlug = (0, lodash_1.kebabCase)(moduleName);
const moduleSnake = (0, lodash_1.snakeCase)(moduleName).toUpperCase();
const language = options.js ? 'js' : 'ts';
const extension = language;
const parseField = (field) => {
    const [rawName, rawType = 'string', flag] = field.split(':');
    const name = (0, lodash_1.camelCase)(rawName.trim());
...
const ensureDirectories = () => {
    const basePath = path_1.default.resolve(process.cwd(), 'src', 'modules', moduleCamel);
    const directories = [
        basePath,
        path_1.default.join(basePath, 'model'),
        path_1.default.join(basePath, 'routes'),
        path_1.default.join(basePath, 'controllers'),
        path_1.default.join(basePath, 'interfaces'),
        path_1.default.join(basePath, 'services'),
        path_1.default.join(basePath, 'validators'),
        path_1.default.join(basePath, 'messages'),
        path_1.default.join(basePath, 'tests'),
    ];
...
const writeFile = (filePath, content) => {
    if (!options.force && fs_1.default.existsSync(filePath)) {
...
const generateController = (type) => {
    const controllerName = `${(0, lodash_1.upperFirst)(type)}${modulePascal}Controller`;
    if (options.lite) {
        if (language === 'js') {
...
const updateRootRouter = () => {
    const routerPath = path_1.default.resolve(process.cwd(), 'src', 'routes', 'index.ts');
    ...
    let content = fs_1.default.readFileSync(routerPath, 'utf8');
    ...
    fs_1.default.writeFileSync(routerPath, content, 'utf8');
...
const main = () => {
    ...
    const webSelected = options.web || (!options.mobile && !options.web);
    const files = {
        [path_1.default.join(moduleBase, `model/${moduleCamel}.model.${extension}`)]: generateModel(),
        [path_1.default.join(moduleBase, `interfaces/${moduleCamel}.interface.${extension}`)]: generateInterface(),
        [path_1.default.join(moduleBase, `messages/message.${extension}`)]: generateMessage(),
        [path_1.default.join(moduleBase, `services/${moduleCamel}.service.${extension}`)]: generateService(),
        [path_1.default.join(moduleBase, `validators/${moduleCamel}.validator.${extension}`)]: generateValidator(),
        [path_1.default.join(moduleBase, 'README.md')]: generateReadme(),
        [path_1.default.join(moduleBase, `tests/${moduleCamel}.routes.spec.${extension}`)]: generateTest('routes'),
        [path_1.default.join(moduleBase, `tests/${moduleCamel}.service.spec.${extension}`)]: generateTest('service'),
        [path_1.default.join(moduleBase, `routes/route.${extension}`)]: generateMainRoute(),
    };
    if (mobileSelected) {
        files[path_1.default.join(moduleBase, `controllers/mobile${modulePascal}.controller.${extension}`)] = generateController('mobile');
        files[path_1.default.join(moduleBase, `routes/mobile${modulePascal}Route.${extension}`)] = generateRoute('mobile');
    }
    if (webSelected) {
        files[path_1.default.join(moduleBase, `controllers/web${modulePascal}.controller.${extension}`)] = generateController('web');
        files[path_1.default.join(moduleBase, `routes/web${modulePascal}Route.${extension}`)] = generateRoute('web');
    }
    if (options.admin) {
        files[path_1.default.join(moduleBase, `controllers/admin${modulePascal}.controller.${extension}`)] = generateController('admin');
    }
    const timestamp = new Date().toISOString().replace(/[-:T.Z]/g, '').slice(0, 14);
    const seederFile = path_1.default.resolve(process.cwd(), 'seeders', `${timestamp}-${moduleSlug}.seed.${extension}`);
