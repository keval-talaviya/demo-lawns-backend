        ...
        return new Promise(async (resolve, reject) => {
            try {
                const [franchise, customer, companySettings] = await Promise.all([
                    user_model_1.UserModel.findById(invoice.franchiseId).lean(),
                    customer_model_1.CustomerModel.findById(invoice.customerId).lean(),
                ...
                doc.fillColor(primaryColor).text(invoice.issuedDate ? new Date(invoice.issuedDate).toLocaleDateString('en-NZ', {
                    day: '2-digit', month: 'short', year: 'numeric'
                }) : new Date().toLocaleDateString('en-NZ', { day: '2-digit', month: 'short', year: 'numeric' }), metaValueX, metaY + 15, { width: 100 });
                metaY += 35;
                ...
                const franchisePhone = franchise?.phoneNumber || '';
                ...
                doc.font('Helvetica');
                const accountNumber = franchise?.accountNumber || process.env.BANK_ACCOUNT_NUMBER || '';
                ...
                const bankAccount = companySettings?.accountNumber || '';
                if (bankAccount) {
